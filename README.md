...
- keeps Markdown lessons readable by rendering headings/prose directly, keeping language code fences top-level so syntax highlighting works, avoiding nested triple-backtick Markdown fences, writing formulas in terminal-readable plain text/Unicode instead of raw `$$` LaTeX blocks, and sanitizing accidental assistant LaTeX display blocks before they are shown,
