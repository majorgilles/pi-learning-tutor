...
- renders common LaTeX math delimiters (`$...$`, `$$...$$`, `\(...\)`, and `\[...\]`) as terminal-friendly Unicode in learning responses and definition overlays without mutating stored assistant/thinking messages, for example `\mathcal{L}(\hat{y}^{(i)}, y^{(i)})` previews as `𝓛(ŷ⁽ⁱ⁾, y⁽ⁱ⁾)` and display formulas show as plain rendered math such as `J(w,b) = 1/m Σᵢ₌₁ᵐ 𝓛(ŷ⁽ⁱ⁾, y⁽ⁱ⁾)`,
