...
- keeps Markdown lessons readable by rendering headings/prose directly, keeping language code fences top-level so syntax highlighting works, avoiding nested triple-backtick Markdown fences, writing formulas in terminal-readable plain text/Unicode instead of raw `$$` LaTeX blocks, and sanitizing accidental assistant LaTeX display blocks into styled Markdown formula callouts before they are shown,
...
- recognizes read-only commands even when their arguments contain shell-like separators inside quotes, shell-local setup like `cd <repo> && PYTHONIOENCODING=utf-8 python ...`, or Python/Node/Bun here-doc inspection snippets, so notebook/file queries are not mistaken for mutating shell pipelines and blocked,
