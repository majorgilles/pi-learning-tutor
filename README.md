...
- renders common LaTeX math delimiters (`$...$`, `$$...$$`, `\(...\)`, and `\[...\]`) as terminal-friendly Unicode in learning responses and definition overlays, for example `\mathcal{L}(\hat{y}^{(i)}, y^{(i)})` becomes `𝓛(ŷ⁽ⁱ⁾, y⁽ⁱ⁾)` and `J(w,b) = \frac{1}{m}\sum_{i=1}^{m} \mathcal{L}(\hat{y}^{(i)}, y^{(i)})` becomes `J(w,b) = 1/m Σᵢ₌₁ᵐ 𝓛(ŷ⁽ⁱ⁾, y⁽ⁱ⁾)`,
