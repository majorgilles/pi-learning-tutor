...
Whenever the extension is loaded, even outside learning mode, assistant LaTeX that the terminal can't render is sanitized into terminal-readable plain text/Unicode before it is shown: display blocks (`$$…$$`, `\[…\]`) become a styled `> **Formula**` callout, and inline `\(…\)` math becomes an inline code span. This keeps math like `$$n = 64 \times 64 \times 3 = 12{,}288 \text{ features}$$` from printing as raw LaTeX.
...
- keeps Markdown lessons readable by rendering headings/prose directly, keeping language code fences top-level so syntax highlighting works, avoiding nested triple-backtick Markdown fences, and writing formulas in terminal-readable plain text/Unicode instead of raw `$$` LaTeX blocks,
