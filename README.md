...
- renders common LaTeX math delimiters (`$...$`, `$$...$$`, `\(...\)`, and `\[...\]`) as highlighted terminal-friendly Unicode previews in learning responses and definition overlays, for example `\mathcal{L}(\hat{y}^{(i)}, y^{(i)})` previews as `⟪ 𝓛(ŷ⁽ⁱ⁾, y⁽ⁱ⁾) ⟫` and display formulas get a `Formula` callout such as `⟪ J(w,b) = 1/m Σᵢ₌₁ᵐ 𝓛(ŷ⁽ⁱ⁾, y⁽ⁱ⁾) ⟫`,
