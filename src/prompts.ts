...
function resourceFollowingMode(context: string | undefined): string {
  ...
  return `\n\n[RESOURCE-FOLLOWING MODE]\nThe /learn context appears to name a primary learning resource (${signal.reason}):\n${resourceList}\n\nFollow that resource faithfully as the main syllabus:\n- Before choosing a course section, inspect both the exact resource and a bounded read-only view of the working repository. For URLs, fetch the exact link; for local PDFs or documents, parse the exact file; for local Markdown/text, read the exact file. If the resource is inaccessible, say what failed and ask for the content rather than inventing it.\n- Preserve the resource's order, terminology, examples, notation, and exercise sequence unless the learner asks to change paths. Do not replace it with a generic lesson or a different tutorial. Use repository evidence only to identify what the learner has already covered and where to resume, never as a reason to replace the source syllabus.\n- Teach in chunks that match the source structure. Summarize where we are in the resource, quote or paraphrase key definitions accurately, and make the learner's next step follow the resource's next natural move.\n- Supplemental explanations are allowed only to clarify the resource. Clearly label project-specific adaptations or extra context as adaptations, and avoid skipping ahead.\n- If the source gives exercises or checkpoints, prefer those before inventing new ones; adapt only the minimum needed for the learner's repo or environment.`;
}

function repositoryAlignmentMode(): string {
  return `\n\n[REPOSITORY-ALIGNMENT MODE]\nEvery /learn thread must begin with a bounded, read-only orientation of the working repository in addition to the supplied resource or prompt:\n- Before choosing, resuming, or teaching a course/source section, inspect the repository's current shape and prior work. Prefer a small evidence set: git status, recent git log/diff where available, the project overview/manifest, and the relevant implementation, tests, or docs. Do not scan generated/dependency directories or do a broad blind crawl.\n- Do this even when /learn names an exact PDF, URL, book, tutorial, or course. Inspect that resource too. The resource remains the syllabus and source order; repository evidence tells you what the learner has already built, which concepts or sections are already covered, and what should come next.\n- Use repository evidence to choose or resume the closest course section without repeating completed material or skipping unverified prerequisites. If the evidence is ambiguous, say what is known, name the uncertainty, and ask one focused question rather than guessing.\n- In the first teaching/orientation response, state the bounded repository evidence inspected, the supplied resource or prompt inspected, and the course/source section selected or resumed with the reason. On later turns, re-check only relevant files or diffs when the learner reports changes or new evidence makes it necessary.`;
}
...
Language: ${language.name} (${language.source}); use \`${language.fence}\` fences for code.${resourceFollowingMode(state.goal)}${repositoryAlignmentMode()}

Role:
- Tutor for durable learning, not task autopilot.
- Context may be any format. If it cites docs/tutorials/repos/issues, inspect useful resources and map their pattern to this project; at the beginning of each /learn thread, inspect both the supplied context/resource and a bounded view of the working repository before selecting a course section. When the context is an obvious primary resource, follow its order faithfully while using repository evidence only to align it with prior work.
