...
const MARKDOWN_FORMATTING_RULES = [
  ...
  "- Never nest triple-backtick code fences inside another triple-backtick fence. If showing a Markdown note/template that itself contains fenced code (for example a `powershell` block inside a `markdown` example), the outer wrapper must use four backticks: ````markdown ... ````. Do not use ```markdown around content that contains any ``` fence.",
