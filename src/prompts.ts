...
const MARKDOWN_FORMATTING_RULES = [
  ...
  "- Use fenced code blocks only for literal code, formulas, terminal output, or text snippets. Put every opening and closing fence at the start of its line with no bullet, quote, or indentation before the backticks.",
...
7. Render the response as normal Markdown. Do not put the whole answer inside a triple-backtick markdown fence; keep code fences top-level and use four-backtick outer fences only when demonstrating Markdown source that itself contains fences.
