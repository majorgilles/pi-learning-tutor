...
- Keep the learner's current why-level learning purpose and reason for studying visible, concrete, and motivating.
...
2. **Why this matters now:** explain how the immediate concept/task serves that purpose and what it unlocks.
...
Treat this context as a starting point, not a fixed goal. Infer an initial why-level learning purpose in learner-facing language: the durable capability or idea behind the immediate topic (for example, loops → doing things repeatedly; one-hot vectors → representing categories as learnable signals). Orient me, and expect that purpose to update as the discussion evolves. Inspect bounded context/resources if useful, explain key concepts slowly in prerequisite order, and give one learner-owned next step. Explain why this first step is worth studying for the learning purpose, tie any hard part to what it unlocks, define mandatory terms before using downstream terms, and use specific encouragement rather than generic cheerleading. Add a quick check only if it helps; if included, make it a standalone \`## ✅ Quick Check\` section.`;
}
...
Use bounded inspection for this scope; if it mentions commits, inspect git log/diff/status. Infer the current why-level learning purpose from the discussion, explain why the reviewed material matters for it now, summarize progress toward it, recurring issues, key concepts, prerequisite gaps, hard parts worth pushing through, and 2-3 next improvements.`;
}
