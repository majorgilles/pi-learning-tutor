...
- Give one learner-owned step at a time. Before typing, name 1-3 concepts and why they matter here.
...
Response:
1. Brief orientation.
2. **Concepts behind this step**: 1-3 bullets tied to the next code/command.
...
Before the next step, inspect bounded read-only context (prefer git status/diff), read only relevant files, state what you inspected, then give concise review: good, improve, next learner-owned step. Include concepts before any next typing step. If this was a quick-check answer, evaluate it with the tutor-check rules.`;
}
...
Orient me, inspect bounded context/resources if useful, explain key concepts, and give one learner-owned next step. Add a quick check only if it helps.`;
}
...
Use bounded evidence (recent commits/diffs/status, issue/goal, resources, conversation) to choose the key concept(s). Then propose one substantial, scoped build challenge where I create a new artifact: feature, component, command, test harness, integration slice, example app, or similar.

Not a tiny drill, short question, prediction, or one-line edit.

Include: evidence used, concepts assessed, why they matter, goal, constraints, target outcome, milestones, success criteria, and hints. If resources exist, adapt their relevant pattern to this project. End with an open invitation to build/share whatever is useful for review; no rigid template or closed fields like "Ready for review", "What I built", or "Files I changed". No solution unless I get stuck after a retry.`;
}
...
Use bounded inspection for this scope; if it mentions commits, inspect git log/diff/status. Summarize progress, recurring issues, key concepts, and 2-3 next improvements.`;
}
...
Then explain what changed, the concepts behind it, and the next learner-owned step.`;
}
