...
const MARKDOWN_FORMATTING_RULES = [
  "Markdown, LaTeX, and code block formatting:",
  ...
  "- Use fenced code blocks only for literal code, terminal output, or text snippets. Put every opening and closing fence at the start of its line with no bullet, quote, or indentation before the backticks.",
  "- For display math or formulas that should render as LaTeX, use `$$` delimiters instead of code fences: put the opening `$$` on its own line, the LaTeX body on following line(s), and the closing `$$` on its own line.",
  "- Keep LaTeX blocks static and self-contained. Do not ask the learner to refresh, reload, clear, or otherwise redraw the terminal to make math appear; the terminal renderer should handle `$$...$$` blocks directly.",
...
7. Render the response as normal Markdown. Do not put the whole answer inside a triple-backtick markdown fence; keep language code fences top-level so they syntax-highlight as code rather than yellow Markdown-source text. Use \`$$\` blocks for display LaTeX instead of code fences, and never ask for a terminal refresh/reload to show math. Use four-backtick outer fences only when demonstrating Markdown source that itself contains fences.
...
Include: evidence used, inferred learning purpose, one beginner-friendly now/later paragraph of 3-4 short sentences/lines about why this challenge is useful, concept ladder/prerequisites assessed, why they matter, the hard part this challenge practices, constraints, target outcome, milestones, success criteria, and hints. Do not assess downstream terms until the required earlier concepts are clear. If resources exist, adapt their relevant pattern to this project. Render headings/prose directly as Markdown, do not wrap the challenge in a triple-backtick markdown fence, keep any code fences at column 0 so language code syntax-highlights correctly, and use standalone \`$$\` blocks for display LaTeX formulas without asking for terminal refresh/reload. End with an open invitation to build/share whatever is useful for review, and say the learner can tell you in their own words when the build artifact is ready; no exact trigger phrase list, no internal review tag, and no rigid template or closed labeled fields. No solution unless I get stuck after a retry.`;
}
